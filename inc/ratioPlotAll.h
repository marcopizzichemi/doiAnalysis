TH1F* ratioPlotAll(int j,Crystal_t (&Crystal)[64],Input_t (&Input)[16],Params_t &Params,TChain *chain)
{
  //gStyle->SetOptStat(0);
  //gStyle->SetOptFit(1);
  std::cout << "Ratio All " << std::endl;
  std::stringstream tempStream;
  tempStream.str("");
  tempStream << "CanvasRatioAll" << j ;
  Crystal[j].CZratioAll = new TCanvas(tempStream.str().c_str(),"");
  tempStream.str("");
  tempStream << "RatioAll" << j;
  ...
  chain->Draw(tempStream.str().c_str(),Crystal[j].crystalCut+Params.triggerPhotopeakCut/*+Params.zCut[z]*/+Crystal[j].ZcrystalPeakCut[0]+Crystal[j].ZcrystalPeakCut[2]+Crystal[j].ZcrystalPeakCut[2]+Crystal[j].ZcrystalPeakCut[3]+Crystal[j].ZcrystalPeakCut[4]);
  tempStream.str("");
